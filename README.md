...
## DuckDNS and Dynu Support

This is a fork of linuxserver/duckdns that adds support for Dynu as well as DuckDNS. It will update the IP address every 5 minutes with some jitter to both hosts. This project is not affiliated with linuxserver.io.

### Environment Variables
```bash
SUBDOMAINS=myduckdns,otherduckdns # DuckDNS subdomains without the duckdns.org part, required
TOKEN=yourtoken # DuckDNS token, required
DYNU_HOST=dynuhost.mywire.org,somehost.ooguy.com # Dynu hosts full name, optional
DYNU_PASS=md5_of_your_dynu_password_or_plaintext # Dynu password, required for Dynu
```
- if your duckdns host is subdomain.duckdns.org, you should set `SUBDOMAINS=subdomain`, not `subdomain.duckdns.org`, comma separated without spaces
- DuckDNS token is found at your account dashboard
- to enter Dynu hosts, you need to enter the full host name, including the dynu domain (eg. 'mywire.org')
- `DYNU_PASS` is the same password that you use to login to Dynu, alternatively you can create a dedicated IP update password [here](https://www.dynu.com/ControlPanel/ManageCredentials)
- you can also hash the password using `echo -n "yourpassword" | md5sum` and use the hash as the password or use this [website](https://www.md5hashgenerator.com)

Consult the [official documentation](#parameters) for more configuration information.

### Debug

This fork also include a simple debug script for troubleshooting purposes.

```bash
docker exec -it duckdns /app/debug.sh
```
The debug script will print out the IP address of the ddns hostname resolved from Cloudflare as such.

```bash
[Debug] IP address of the server: xxx.xxx.xxx.xxx.
[DuckDNS]: host1.duckdns.org has IP address of xxx.xxx.xxx.xxx.
[DuckDNS]: host2.duckdns.org has IP address of xxx.xxx.xxx.xxx.
[DuckDNS]: host3.duckdns.org has IP address of xxx.xxx.xxx.xxx.
[Dynu]: newserver.giize.com has IP address of xxx.xxx.xxx.xxx.
```

# Official Documentation from Linuxserver
